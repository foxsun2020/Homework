...
text = open(r'C:\Users\fox\Documents\休闲娱乐互骂消遣.txt', encoding='utf-8')  # open source chat text file
...
for s in str_s:
    if s.find('[表情]') != -1:
        s = s.replace('[表情]', '')
            ...
            or s.find('清') != -1 or s.find('屏') != -1 or s.find('专') != -1 or s.find('神') != -1 \
            or s.find('用') != -1 or s.find('定') != -1 or s.find('针') != -1 or s.find('海') != -1 \
...
cloud_mask = np.array(Image.open(r'C:\Users\fox\Pictures\1234.png'))  # set background and shape
